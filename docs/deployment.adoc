...
4. Save changes
5. Push to master/main to trigger deployment

The workflow automatically creates and updates the `gh-pages` branch with the contents of the `html/` directory.

**Troubleshooting:**

* If deployment fails, ensure Actions have write permissions (Settings → Actions → General → Workflow permissions → "Read and write permissions")
* The first deployment creates the `gh-pages` branch automatically
